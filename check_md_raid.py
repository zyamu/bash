...
def find_arrays(verbosity):
    ...
    devices_output = os.popen("sudo -S %s --detail --scan" % BIN).readlines()
    raid_devices   = []
...
def test_raid(verbosity):
    ...
    for array in raid_devices:
        ...
        detailed_output = os.popen("sudo -S %s --detail %s" % (BIN, array) ).readlines()
        ...
        if not re_clean.match(state) and state != "active" and state != "active, checking":
            arrays_not_ok += 1
